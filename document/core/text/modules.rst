...
.. math::
   \begin{array}{llclll}
   \production{module field} &
     \text{(}~\text{memory}~~\Tid^?~~it^?~~\text{(}~\text{data}~~b^n{:}\Tdatastring~\text{)}~~\text{)} \quad\equiv \\ & \qquad
       \text{(}~\text{memory}~~\Tid'~~it'~~m~~m~\text{)} \\ & \qquad
       \text{(}~\text{data}~~\text{(}~\text{memory}~~\Tid'~\text{)}~~\text{(}~it'~\text{.const}~~\text{0}~\text{)}~~\Tdatastring~\text{)}
       \\ & \qquad\qquad
       (\iff \Tid^? \neq \epsilon \wedge \Tid' = \Tid^? \vee \Tid^? = \epsilon \wedge \Tid' \idfresh, \\ & \qquad\qquad
        \iff it^? \neq \epsilon \wedge it' = it^? \vee it^? = \epsilon \wedge it' = \text{i32}, \\ & \qquad\qquad
        m = \F{ceil}(n / 64\,\F{Ki})), \\
   \end{array}
